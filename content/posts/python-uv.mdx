...
<Image src="/thumbnails/python-logo.png" alt="Python uv" />
...
`uv run` 은 말 그래도 무엇인가 실행할 때 사용하는 명령어 입니다. `uv run` 명령어를 통하면 uv에서 정의된 프로젝트 환경을 자동으로 설정하고 입력된 명령이 실행됩니다. 이 말은 굳이 가상환경을 activate할 필요가 없다는 뜻이기도 합니다.
