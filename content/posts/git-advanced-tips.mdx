...
description: 프로젝트가 복잡해지고 협업이 많아질수록 Git을 제대로 활용하는 능력이 중요해집니다. Git을 더 효율적으로 사용할 수 있는 팁과 실수를 예방하기 위한 팁을 소개하겠습니다.
tags: [Git]
author: EKO
...
Git은 현대 소프트웨어 개발에서 필수적인 버전 관리 도구입니다. 프로젝트가 복잡해지고 협업이 많아질수록 Git을 제대로 활용하는 능력이 중요해집니다. 이 글에서는 Git의 기본 개념과 `add`, `commit`, `push` 등 기본 명령어를 알고 있다는 가정 하에, Git을 더 효율적으로 사용할 수 있는 팁과 실수를 예방하기 위한 팁을 소개하겠습니다.
...
<Image src="/git-advanced-tips/01.jpg" alt="Git Graph Extension" />
...
만약 `git reset`으로 커밋을 잘못 삭제했다면 복구할 수 있을까요? 특히나 `git reset --hard`로 커밋을 완전히 날려버렸다가 실수한 걸 깨달았다면... 정녕 돌이킬 방법이 없을까요?

다행히도 복구가 가능합니다. `git reflog` 명령어는 reference log의 줄임말로, **HEAD의 이동 기록**을 조회하는 명령어입니다. `git log` 명령어와 달리 reset이나 rebase 등으로 변경된 HEAD의 위치까지 모두 조회할 수 있습니다. `git reflog`로 삭제된 커밋의 해시값을 확인한 후, `git reset --hard`로 복구하면 됩니다.
...
`git cherry-pick`은 브랜치를 merge하지 않고도 **특정 커밋만 현재 브랜치로 가져올 수 있는** 명령어입니다. 실수로 잘못된 브랜치에 커밋을 한 경우나, 다른 브랜치에서 버그 수정 혹은 설정 변경 등을 한 변경사항을 빠르게 적용하고 싶을 때 유용합니다.

```bash
git cherry-pick <commit-hash>
```

## 9. `git branch -D`: 로컬 브랜치 삭제하기

브랜치가 성공적으로 merge되었거나 더 이상 필요하지 않은 경우 삭제하는 것이 좋습니다. 브랜치가 많이 쌓이면 히스토리가 복잡해지고 관리가 어려워지기 때문입니다. GitHub에서 PR을 merge한 후 삭제를 하더라도, 로컬 브랜치는 수동으로 삭제해야 합니다.

```bash
git branch -D <branch-name>
```

## 10. `git remote prune origin`: 원격 브랜치 정리하기

원격 저장소에서 삭제된 브랜치가 로컬에서 여전히 추적되는 경우가 있습니다. 이럴 땐 `git remote prune origin` 명령어로 로컬에서 추적 중인 원격 브랜치를 정리할 수 있습니다. 로컬과 원격 저장소 상태를 동기화하여 불필요한 브랜치를 제거하면 git graph가 깔끔해집니다.

```bash
git remote prune origin
```

원격 저장소에서 삭제된 브랜치를 로컬에서 제거하는 것이므로, 로컬에서 작업 중인 브랜치는 영향을 받지 않습니다.
